...
// Helper to verify if user has Administrator authority
export const isAdminUser = (user?: UserProfile | null): boolean => {
  if (!user || !user.role) return false;
  const role = user.role.toUpperCase();
  return role === 'ADMIN' || role === 'ROLE_ADMIN' || role === 'SUPER_ADMIN';
};
...
import { CredentialEdgePage } from './pages/CredentialEdgePage';
import { CredentialCourseDetailPage } from './pages/CredentialCourseDetailPage';
...
function AppContent() {
  ...
  const [currentPath, setCurrentPath] = useState<string>(() => {
    ...
    // If an unauthenticated user with NO token directly visits a protected route, redirect to '/login'
  ...
  // Protect admin routes: If a normal user manually opens /admin, deny access and redirect to home
  useEffect(() => {
    if (currentPath.startsWith('/admin') && !isAuthLoading) {
  ...
  const handleAuthSuccess = (user: UserProfile) => {
    ...
    // Retrieve and clear any intended redirect path
    ...
    // 1. Admin Login Redirect: If administrator successfully logs in, redirect to /admin
    if (isAdmin) {
      const adminPath = targetRedirect && targetRedirect.startsWith('/admin') ? targetRedirect : '/admin';
      showToast(`Welcome Administrator ${user.name}! Redirecting to Admin Control Center...`);
      navigate(adminPath);
      return;
    }

    // 2. Normal User Login: Normal learner/user login must NOT redirect to /admin
    if (
      targetRedirect &&
      ...
      targetRedirect !== '/' &&
      !targetRedirect.startsWith('/admin')
    ) {
  ...
  const handleLogout = async () => {
    const wasAdmin = currentPath.startsWith('/admin');
    await logout();
    setRedirectAfterAuth(null);
    showToast('You have signed out successfully.');
    if (wasAdmin || isProtectedRoute(currentPath)) {
      navigate('/login');
    }
  };
  ...
  const navigate = (path: string, param?: string) => {
    ...
    if (isProtectedRoute(targetPath) && !token && !isAuthenticated && !isAuthLoading) {
      ...
      const fallbackUrl = targetPath.startsWith('/admin') ? '/login' : '/';
      try {
  ...
  const renderCurrentPage = () => {
    ...
    if (currentPath.startsWith('/admin')) {
      ...
      if (!currentUser) {
        return (
          <ProtectedRoute
            ...
            pageTitle="Ingage Admin Control Center"
      ...
      if (currentPath === '/admin' || currentPath === '/admin/dashboard') {
        ...
        adminContent = <AdminUsersPage onNavigate={navigate} currentUser={currentUser} />;
      } else if (currentPath === '/admin/instructors') {
        adminContent = <AdminUsersPage onNavigate={navigate} currentUser={currentUser} initialRoleFilter="INSTRUCTOR" />;
      } else if (currentPath.startsWith('/admin/users/')) {
        const userId = currentPath.replace('/admin/users/', '') || '1';
        ...
        adminContent = <AdminCategoriesPage onNavigate={navigate} />;
      } else if (currentPath === '/admin/courses') {
        adminContent = <AdminCoursesPage onNavigate={navigate} />;
      } else if (currentPath === '/admin/credential-edge') {
        adminContent = <AdminCredentialEdgePage />;
      ...
      } else if (currentPath === '/admin/progress' || currentPath === '/admin/enrollments') {
      ...
      } else if (currentPath === '/admin/opportunities') {
        adminContent = <AdminOpportunitiesPage onNavigate={navigate} />;
      } else if (currentPath === '/admin/projects') {
        adminContent = <AdminProjectsPage onNavigate={navigate} />;
      } else if (currentPath === '/admin/activity' || currentPath === '/admin/notifications') {
        adminContent = <AdminActivityLogsPage />;
      } else if (currentPath === '/admin/settings') {
        adminContent = <AdminSettingsPage onNavigate={navigate} currentUser={currentUser} onShowToast={showToast} />;
      } else {
        adminContent = <AdminDashboardPage onNavigate={navigate} />;
      }
    ...
    // Public Certificate Verification Hub
    if (
      currentPath.startsWith('/certificate/verify') ||
      currentPath.startsWith('/certificates/verify') ||
      currentPath.startsWith('/verify/certificate')
    ) {
      const parts = currentPath.split('/verify/');
      const code = parts.length > 1 ? parts[1].split('?')[0].split('#')[0] : '';
      return <CertificateVerificationPage code={code} onNavigate={navigate} />;
    }

    // Individual Certificate View & Download
    if (currentPath.startsWith('/certificates/') && !currentPath.includes('/verify')) {
      const certId = currentPath.slice(14).split('?')[0].split('#')[0];
      return (
    ...
    if (currentPath.startsWith('/verify-otp') || currentPath.startsWith('/login/verify-otp') || currentPath.startsWith('/verify-email')) {
    ...
    if (currentPath === '/credential-edge') {
      return (
        <CredentialEdgePage
          onNavigate={navigate}
          currentUser={currentUser}
    ...
    if (currentPath === '/credential-edge/credentials') {
      return (
        <MyCredentialsPage
          onNavigate={navigate}
          currentUser={currentUser}
    ...
    if (currentPath.startsWith('/credential-edge/')) {
      const credSlug = currentPath.slice(17).split('?')[0].split('#')[0];
      return (
    ...
    if (currentPath.startsWith('/profile')) {
      let initialTab: string = 'overview';
      if (currentPath === '/profile/certificates' || currentPath === '/profile/credentials') initialTab = 'credentials';
      else if (currentPath === '/profile/compass') initialTab = 'compass';
      else if (currentPath === '/profile/learning') initialTab = 'learning';
      else if (currentPath === '/profile/projects') initialTab = 'projects';
