...
export function getAccessibleImageUrl(path?: string | null): string {
  ...
  if (clean.startsWith('http://') || clean.startsWith('https://') || clean.startsWith('data:') || clean.startsWith('blob:')) {
    try {
      ...
      // Fix cases where backend port 8080/8000 URL was created with /uploads instead of /api/uploads
      if ((url.port === '8080' || url.port === '8000' || url.hostname === 'localhost') && url.pathname.startsWith('/uploads/')) {
