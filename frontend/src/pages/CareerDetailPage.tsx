...
import { CareerDetailDto, CareerCourseDto } from '../api/careerApi';
...
export function CareerDetailPage({
  ...
  const isCareerCompleted = Boolean(
    ...
    career.courses.every((c) => c.completed || c.enrollmentStatus === 'COMPLETED')
  );
  ...
  const handleDownloadSyllabus = () => {
    const textContent =
      `Ingage LMS - ${career.title} Career Curriculum\n` +
      `Duration: ${career.duration}\n` +
      `Level: ${career.level}\n` +
      `Average Salary: ${career.salary?.formatted || 'Competitive'}\n` +
      `Job Openings: ${career.jobOpenings}\n\n` +
      `Modules:\n` +
      modulesList
        .map((m) => `${m.number}. ${m.title} (${m.lessonsCount} lessons • ${m.duration})`)
        .join('\n') +
      `\n\nKey Skills: ${skillsList.join(', ')}\n` +
      `Certification: ${career.certificationName || `Certified ${career.title} Professional`}\n`;
  ...
  // Handle Main Enrollment or Sign up
  const handleEnrollClick = () => {
    const targetUrl = `/learn/${career.slug}`;
    if (currentUser) {
      if (isEnrolledInCareer || isCareerCompleted) {
        onNavigate(targetUrl, career.slug);
        return;
      }
      // If logged in but not yet enrolled in any course, scroll to courses or navigate to curriculum
      const coursesSection = document.getElementById('recommended-courses-section');
      if (coursesSection) {
        coursesSection.scrollIntoView({ behavior: 'smooth' });
        if (onShowToast) {
          onShowToast('Select a course below to complete your enrollment.');
        }
      } else {
        onNavigate(targetUrl, career.slug);
      }
            ...
            <div className="lg:col-span-7 space-y-5">
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-3.5 pt-4">
                <button
                  ...
                  className="w-full sm:w-auto bg-lime-600 hover:bg-lime-700 text-white font-bold text-sm sm:text-base px-6 sm:px-8 py-3 rounded-lg shadow-xs transition-colors cursor-pointer text-center flex items-center justify-center gap-2"
                >
                  {!currentUser
                    ? 'Sign Up to Continue'
                    : isCareerCompleted
                ...
                <button
                  ...
                  onClick={() => onNavigate(`/learn/${career.slug}`, career.slug)}
              ...
              <div id="recommended-courses-section">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  {career.courses.map((c) => {
                    ...
                    const isEnrolled = c.courseAccess || c.enrollmentStatus === 'ENROLLED';
                            ...
                            >
                              {isPurchasing ? (
                                ...
                                <>
                                  ...
                                  <span>Enroll Now (₹{c.coursePrice || c.price})</span>
                                </>
                              )}
                            </button>
                          )}
          ...
          <div className="lg:col-span-4 space-y-6 lg:sticky lg:top-24">
            ...
            <div className="bg-[#F8FAF4] border border-lime-200 rounded-3xl p-6 sm:p-7 shadow-xs">
              ...
              <div className="mt-4 flex items-baseline gap-2.5">
                <span className="text-3xl sm:text-4xl font-extrabold text-gray-900">₹ 14,999</span>
                <span className="text-gray-400 line-through text-lg font-medium">₹ 24,999</span>
              </div>
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-2xs">
              ...
              <button
                ...
                className="w-full py-2.5 px-4 rounded-xl border border-gray-300 text-gray-700 font-semibold text-sm flex items-center justify-center gap-2 hover:bg-gray-50 transition-colors cursor-pointer"
              >
