...
public class CourseService {
    ...
    public java.util.Map<String, Object> getCourseContent(Long id, java.security.Principal principal) {
        ...
        if (principal != null) {
            ...
            if (userOpt.isPresent()) {
                ...
                java.util.Optional<Enrollment> enrollOpt = enrollmentRepository.findByUserIdAndCourseId(user.getId(), id);
                if (enrollOpt.isPresent()) {
                    enrollment = enrollOpt.get();
                    isEnrolled = true;
        ...
        java.util.Map<Long, LessonProgress> progressMap = new java.util.HashMap<>();
        java.util.List<Long> completedLessonIds = new java.util.ArrayList<>();
        if (enrollment != null) {
            java.util.List<LessonProgress> lpList = lessonProgressRepository.findByEnrollmentId(enrollment.getId());
            for (LessonProgress lp : lpList) {
        ...
        result.put("progressPercentage", enrollment != null && enrollment.getProgressPercentage() != null ? enrollment.getProgressPercentage() : 0);
        ...
        for (com.lms.Backend.course.entity.CourseSection sec : sections) {
            ...
            for (com.lms.Backend.course.entity.CourseLesson l : lessons) {
                ...
                int watchDurationSeconds = lp != null && lp.getWatchDurationSeconds() != null ? lp.getWatchDurationSeconds() : 0;
                lesMap.put("completed", isCompleted);
                lesMap.put("watchDurationSeconds", watchDurationSeconds);
    ...
    private CourseResponse mapToResponse(Course course) {
        return new CourseResponse(
                ...
                course.getPrice(),
                course.getFinalPrice() != null ? course.getFinalPrice() : (course.getPrice() != null ? (double) course.getPrice() : 0.0),
                course.getStatus() != null ? course.getStatus().name() : (course.isPublished() ? "PUBLISHED" : "DRAFT"),
                course.isPublished(),
                course.getCreatedAt(),
                course.getUpdatedAt()
        );
    ...
    public CourseEnrollmentStatusResponse getEnrollmentStatus(Long id, Principal principal) {
        ...
        // 4. Career Path Access fallback: check if user owns a Career Path that includes this course
        if (courseAccessService.hasCourseAccess(user.getId(), course.getId())) {
            return new CourseEnrollmentStatusResponse(
                course.getId(),
                true,
                "INCLUDED",
    ...
    public java.util.Map<String, Object> getCourseLearnAccess(Long id, Principal principal) {
        ...
        if (user.getRole() != null && user.getRole().name().equals("ADMIN")) {
            ...
            hasAccess = courseAccessService.hasCourseAccess(user.getId(), id);
        }
