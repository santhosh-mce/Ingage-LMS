...
public class FileUploadService {
    ...
    public String storeFile(MultipartFile file, String category, String prefix) throws IOException {
        ...
        return "/api/uploads/" + subDir + "/" + uniqueFilename;
    }
    ...
    public boolean deleteOldDocument(String fileUrl, String expectedUserPrefix) {
        if (fileUrl == null || fileUrl.isBlank()) {
            return false;
        }
        try {
            ...
            if (expectedUserPrefix != null && !expectedUserPrefix.isBlank()) {
                if (!fileName.startsWith(expectedUserPrefix) && !fileName.startsWith("user-")) {
                    log.warn("[FileUploadService] Document '{}' does not match user prefix '{}'. Skipping deletion.", fileName, expectedUserPrefix);
                    return false;
                }
            }

            Path destinationFolder = rootLocation.resolve("documents").normalize();
            Path filePath = destinationFolder.resolve(fileName).normalize();

            if (filePath.startsWith(destinationFolder) && Files.exists(filePath) && Files.isRegularFile(filePath)) {
                boolean deleted = Files.deleteIfExists(filePath);
                log.info("[FileUploadService] Successfully deleted old document: {} (success: {})", filePath, deleted);
                return deleted;
            }
        } catch (Exception e) {
            log.warn("[FileUploadService] Could not delete old document '{}': {}", fileUrl, e.getMessage());
        }
        return false;
    }
