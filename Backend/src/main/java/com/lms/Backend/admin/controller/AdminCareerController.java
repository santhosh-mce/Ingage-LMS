...
public class AdminCareerController {
    ...
    public ResponseEntity<Map<String, Object>> assignCourse(
        ...
        Boolean requiredForCompletion = true;
        if (payload != null) {
            if (payload.containsKey("included")) included = Boolean.parseBoolean(payload.get("included").toString());
            if (payload.containsKey("requiredForCompletion")) requiredForCompletion = Boolean.parseBoolean(payload.get("requiredForCompletion").toString());
        }
        careerService.assignCourseToCareer(id, courseId, included, requiredForCompletion);
