...
public class AdminProjectController {
    ...
    public ResponseEntity<Map<String, Object>> getProjectStats() {
        ...
        long total = all.size();
        long published = all.stream().filter(Project::isPublished).count();
    ...
    @GetMapping("/{id}")
    @org.springframework.transaction.annotation.Transactional(readOnly = true)
    public ResponseEntity<com.lms.Backend.project.dto.ProjectDto> getProjectById(@PathVariable Long id) {
        return ResponseEntity.ok(projectService.getProjectByIdAdmin(id));
    }

    @PutMapping("/{id}")
    public ResponseEntity<com.lms.Backend.project.dto.ProjectDto> updateProject(
