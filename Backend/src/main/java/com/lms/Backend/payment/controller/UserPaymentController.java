...
public class UserPaymentController {
    ...
    public ResponseEntity<Map<String, Object>> createOrder(
        ...
        if (payload.containsKey("careerId")) {
            Long careerId = Long.parseLong(payload.get("careerId").toString());
            Map<String, Object> order = paymentService.createCareerPathOrder(user.getId(), careerId, couponCode);
        ...
        throw new IllegalArgumentException("Course ID or Career ID is required to create payment order.");
    }

    @PostMapping("/orders/career/create")
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<Map<String, Object>> createCareerOrder(
        @RequestBody Map<String, Object> payload,
        Principal principal
    ) {
        User user = userRepository.findByEmailIgnoreCase(principal.getName())
        ...
        if (!payload.containsKey("careerId")) {
            throw new IllegalArgumentException("Career ID is required to create payment order.");
        }
