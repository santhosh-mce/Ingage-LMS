...
public interface PaymentRepository extends JpaRepository<Payment, Long> {
    ...
    Optional<Payment> findFirstByUserIdAndCareerIdOrderByCreatedAtDesc(UUID userId, Long careerId);

    boolean existsByUserIdAndCourseIdAndPaymentStatus(UUID userId, Long courseId, PaymentStatus status);

    boolean existsByUserIdAndCareerIdAndPaymentStatus(UUID userId, Long careerId, PaymentStatus status);
    ...
    @Query("SELECT COALESCE(SUM(p.finalAmount), 0.0) FROM Payment p WHERE p.paymentStatus = 'PAID' AND p.createdAt >= :start AND p.createdAt < :end")
    Double getRevenueBetween(@Param("start") Instant start, @Param("end") Instant end);
}
